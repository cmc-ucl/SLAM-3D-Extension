	...
	for(int l=0;l<5000;l++)
	{
		...
		for(int i=0;i<integral_knot.size()-1;i++)
		{
			...
			for(int k=0;k<grid;k++)
			{
				...
				fa_xxyy = pow(dp + cp*r + bp*pow(r,2.) + ap*pow(r,3.),2.)*EnergyAngularIntegral_real_xxyy(sig,r,d);
				...
				fb_xxyy = pow(dp + cp*r + bp*pow(r,2.) + ap*pow(r,3.),2.)*EnergyAngularIntegral_real_xxyy(sig,r,d);
