...
*/
        //for(int i=0;i<integral_knot.size()-1;i++)
	#pragma omp parallel for default(none) private(integral_knot,Rs,Rp,g) reduction(+:res) num_threads(NUM_OMP_THREAD) schedule(static)
        for(int i=0;i<(int)(integral_knot.size()-1);i++)
        {
		double fa, fb, dr, mesh;
		double r,r_inc;
                dr   = integral_knot[i+1] - integral_knot[i];
